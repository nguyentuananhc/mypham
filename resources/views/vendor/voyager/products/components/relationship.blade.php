    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'hasMany')
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @if($options->model == \App\ProductTranslation::class)
                    @php
                        $allLang = \App\Language::all();
                        $value = $model::where($options->column, '=', $relationshipData->id)
                        ->join('languages', 'languages.code', '=', $options->table . '.lang_code')->get();
                    @endphp
                    @foreach($value as $lang)
                        <a class="btn btn-primary btm-sm">{{ $lang->name }}</a>
                    @endforeach
                    @if(count($allLang) > count($value))
